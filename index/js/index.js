$(function () {
    ...
    function pageScroll(){//滚轮事件函数
        $("#pageBoxs").on("mousewheel", throttle(function (e, delta) {
            if (delta < 0) {//向上滚动时
                pmoved++;
    ...
    docComplete(function(){
        var loadingMask=document.getElementById("l");
    ...
    /*2.公司介绍楼层 图片拖拽*/
    var $aboutImgBox = $("#page2>.about-box>.about-right>.img-box>ul");//图片容器
    var $aboutImgW = 390;//图片显示框的宽度
    var minLeft = -($aboutImgBox.children().length - 1) * $aboutImgW;//向左能拖拽的最大距离
    var s = 650;//控制拖拽力度的参数
    $aboutImgBox.css('width', $aboutImgBox.children().length * $aboutImgW);//设置图片容器的宽度
    ...
    function loadIndexProducts(tid) {
        $.ajax({
            type: "GET",
            url: "data/product/getProducts_index.php",
            data: {tid: tid},
            dataType: "json",
    ...
    function loadNews(id){
        $.ajax({
            type:"GET",
            url:"data/news/getNews_index.php",
            data:{tid:id},
            dataType:"json",
            success:function(data){
